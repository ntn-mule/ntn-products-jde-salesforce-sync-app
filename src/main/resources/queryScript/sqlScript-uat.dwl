...
Website AS (
	...
	FROM CRPDTA.F01151 EA
	JOIN CRPDTA.F0111 WW ON EA.EAAN8 = WW.WWAN8
	WHERE WW.WWIDLN = '0'
	  AND EA.EAIDLN = '0'
	  AND EA.EAETP = 'I'
),
...
billingAddress AS (
	...
	WHERE AL1.ALAN8 in (select f1.ABAN81 from CRPDTA.F0101 f1 where f1.ABUPMJ >= $(vars.jobRun.date) and f1.ABUPMT >= $(vars.previousJobRun.time) AND f1.ABAT1 IN ('C','EU','EUX','CX','N') AND TRIM(f1.ABSIC) = 'NBCC') and (AL1.ALUPMJ >= $(vars.jobRun.date) AND AL1.ALUPMT >= $(vars.previousJobRun.time))
),
...
ShippingAddress AS (
	SELECT AL1.*
	FROM CRPDTA.F0116 AL1
	WHERE AL1.ALAN8 in (select f1.ABAN8 from CRPDTA.F0101 f1 where f1.ABAN8='29814' AND f1.ABAT1 IN ('C','EU','EUX','CX','N') AND TRIM(f1.ABSIC) = 'NBCC') 
	--f1 where f1.ABUPMJ >= $(vars.jobRun.date) and f1.ABUPMT >= $(vars.previousJobRun.time) AND f1.ABAT1 IN ('C','EU','EUX','CX','N') AND TRIM(f1.ABSIC) = 'NBCC') and (AL1.ALUPMJ >= $(vars.jobRun.date) AND AL1.ALUPMT >= $(vars.previousJobRun.time))
),
...
CurrencyIsoCode AS (
	...
	FROM CRPDTA.F03012 f3
	WHERE f3.AIAN8 in (select f1.ABAN8 from CRPDTA.F0101 f1 where f1.ABAN8='29814' AND f1.ABAT1 IN ('C','EU','EUX','CX','N') AND TRIM(f1.ABSIC) = 'NBCC')
	--f1.ABUPMJ >= $(vars.jobRun.date) and f1.ABUPMT >= $(vars.previousJobRun.time)  and (f3.AIUPMJ >= $(vars.jobRun.date) AND f3.AIUPMT >= $(vars.previousJobRun.time))
),
...
F03012table AS (
	...
	WHERE trim(f3.AIASN)  in (select f1.ABAN8 from CRPDTA.F0101 f1 where f1.ABAN8='29814' AND trim(f1.ABAT1) IN ('C','EU','EUX','CX','N') AND TRIM(f1.ABSIC) = 'NBCC')
	 --f1.ABUPMJ >= $(vars.jobRun.date) and f1.ABUPMT >= $(vars.previousJobRun.time) AND trim(f1.ABAT1) IN ('C','EU','EUX','CX','N') AND TRIM(f1.ABSIC) = 'NBCC') and (f2.ABUPMJ >= $(vars.jobRun.date) and f2.ABUPMT >= $(vars.previousJobRun.time)) and (f3.AIUPMJ >= $(vars.jobRun.date) AND f3.AIUPMT >= $(vars.previousJobRun.time))
...
branchCode AS (
    select AXEXRA, AXAN8
    from CRPDTA.F4780
    where AXAN8 in (select f1.ABAN8 from CRPDTA.F0101 f1 where f1.ABUPMJ >= $(vars.jobRun.date) and f1.ABUPMT >= $(vars.previousJobRun.time) AND f1.ABAT1 IN ('C','EU','EUX','CX','N') AND TRIM(f1.ABSIC) = 'NBCC')
),
